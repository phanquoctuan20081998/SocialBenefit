...
class AddCommentSurveyService: APIServiceProtocol {
    ...
    func request(replyTo: CommentResultModel?, surveryId: Int?, content: String, completion: @escaping (Result<AddCommentResultModel, AppError>) -> Void) {
        let header: HTTPHeaders = [
            "token": userInfor.token,
            "employeeId": userInfor.employeeId
        ]
        
        let body = AddCommentRequestModel.init(replyTo: replyTo, surveyId: surveryId, content: content)
